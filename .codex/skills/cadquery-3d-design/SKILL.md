...
* [ ] **Consider optional test prints.** Identify physically uncertain interactions that a small representative print could test economically. Where worthwhile, build and verify test pieces alongside the complete model using the guidance below; do not pause the full design merely to await a trial print.
...
* [ ] **Export and verify deliverables.** Generate the required STEP/STL exports from the same final print-ready layout, verify matching orientation and component placement as required by `AGENTS.md`, check scale and mesh detail, and save useful isometric/front/top/right views. Inspect a slicer preview if available; record whether slicing and trial printing are verified or still outstanding.
